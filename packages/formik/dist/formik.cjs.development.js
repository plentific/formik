...
function useFormik(_ref) {
  ...
  var runValidateHandler = React.useCallback(function (values, field) {
    var maybePromisedErrors = props.validate == null ? void 0 : props.validate(values, field);

    if (!maybePromisedErrors) {
      // use loose null check here on purpose
      return emptyErrors;
    } else if (isPromise(maybePromisedErrors)) {
      return maybePromisedErrors.then(function (errors) {
  ...
  var runFieldLevelValidations = React.useCallback(function (values) {
    ...
    var fieldValidations = fieldKeysWithValidation.map(function (f) {
      return runSingleFieldLevelValidation(f, getIn(values, f));
    });

    var processFieldErrors = function processFieldErrors(fieldErrorsList) {
    ...
    if (fieldValidations.some(isPromise)) {
      ...
      return processFieldErrors(fieldValidations);
    }
  }, [runSingleFieldLevelValidation]); // Run all validations and return the result

  var runAllValidations = React.useCallback(function (values) {
    var allValidations = [runFieldLevelValidations(values), props.validationSchema ? runValidationSchema(values) : {}, props.validate ? runValidateHandler(values) : {}];

    var processAllValidations = function processAllValidations(_ref2) {
  ...
  var validateFormWithHighPriority = useEventCallback(function (values) {
    ...
    var processCombinedErrors = function processCombinedErrors(combinedErrors) {
    ...
    var maybePromisedErrors = runAllValidations(values);
  ...
  }, [props.initialErrors, props.initialStatus, props.initialTouched, props.onReset]); // Grouping the effects to it's easier to predict when they run and synchronizing setting isMounted to true, validation on mount and reinitialization

  React.useEffect(function () {
    if (isMounted.current === false) {
      isMounted.current = true;

      if (validateOnMount && isEqual(initialValues.current, props.initialValues)) {
      ...
      if (!isEqual(initialTouched.current, props.initialTouched || emptyTouched)) {
  ...
  }, [enableReinitialize, props.initialValues, props.initialErrors, props.initialStatus, props.initialTouched, resetForm, validateFormWithHighPriority, validateOnMount]); // Splitting the mounting reset to its own effect so it is only ever called once: when the component really unmounts
  ...
  var submitForm = useEventCallback(function () {
    ...
    var processErrors = function processErrors(combinedErrors) {
    ...
    var maybePromisedErrors = validateFormWithHighPriority();
  ...
  var handleSubmit = useEventCallback(function (e) {
    ...
    var maybePromise = submitForm();

    if (isPromise(maybePromise)) {
      maybePromise["catch"](function (reason) {
    ...
    var C = /*#__PURE__*/function (_React$Component) {
      ...
      function C() {
        ...
        _this.renderFormComponent = function (formikProps) {
          return React.createElement(Component, _extends({}, _this.props, formikProps));
        };
      ...
      _proto.render = function render() {
        ...
        return React.createElement(Formik, _extends({}, props, config, {
